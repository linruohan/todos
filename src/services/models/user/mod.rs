...
fn insert_get_results_batch() {
    ...
    conn.test_transaction::<_, Error, _>(|conn| {
        ...
        let expected_users = vec![
            User(1, "Sean".into(), None, now, now),
            User(2, "Tess".into(), None, now, now),
        ];
...
fn insert_get_result() {
    ...
    conn.test_transaction::<_, Error, _>(|conn| {
        ...
        let now = select(diesel::dsl::now).get_result::<NaiveDateTime>(&mut conn)?;
        ...
        let expected_user = User(3, "Ruby".into(), None, now, now);
